...
def getBondDuration(y, face, couponRate, m, ppy=1):
    ...
    pvcf_t = sum([t * (coupon / (1 + discountRate) ** t) for t in range(1, periods + 1)])
    pvcf_face = periods * (face / (1 + discountRate) ** periods)
    
    bondPrice = getBondPrice(y, face, couponRate, m, ppy)
    bondDuration = (pvcf_t + pvcf_face) / bondPrice
    return bondDuration
...
print(getBondDuration(y, face, couponRate, m, 1))  # Annual payments
